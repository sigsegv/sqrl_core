...
void sqrl_base64(sqrl_strbuf_t *out, const void *binaryData, size_t len)
{
    ...
    flen = 4*(len + pad)/3;
    sqrl_strbuf_create(out, flen + 1 - pad); // we skip padding
    res = out->str;
    out->len = out->cap - 1;
...
void sqrl_unbase64(sqrl_buffer_t *out, const char *ascii, size_t len)
{
    ...
    if(len == 32) pad = 1;
    else if(len == 64) pad = 2;
    
    flen = 3*len/4 - pad ;
    sqrl_buffer_create(out, flen);
    bin = out->ptr;
    if( !bin )
    {
